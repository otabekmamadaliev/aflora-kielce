/* Aflora — skrypt tylko tej strony. Bez zależności. */
(function () {
  'use strict';
  ...
  var pasek = document.querySelector('.szyld');
  if (pasek) {
  ...
  if ('IntersectionObserver' in window && !spokojnie) {
    var cele = document.querySelectorAll('.bukiet, .o-kwiaciarni p, .obietnice li, .kadry figure, .kontakt-kartka');
    var io = new IntersectionObserver(function (es) {
      es.forEach(function (en) {
        if (!en.isIntersecting) return;
        en.target.style.transition = 'opacity .65s ease, transform .65s cubic-bezier(.2,.7,.3,1)';
  ...
  /* Dzisiejsza pozycja w godzinach. Liczy przegladarka. */
  (function(){
    var i=(new Date().getDay()+6)%7;
    var el=document.querySelector('.dzien-poz[data-dzien="'+i+'"]');
